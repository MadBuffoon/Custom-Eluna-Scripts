...
local T = {
    ...
    [6] = {
        -- Legs
        {"Enchant Legs|r - Earthen Leg Armor", 3853, false},
        {"Enchant Legs|r - Frosthide Leg Armor", 3822, false},
        {"Enchant Legs|r - Icescale Leg Armor", 3823, false},
        {"Enchant Legs|r - Brilliant Spellthread", 3719, false},
        {"Enchant Legs|r - Sapphire Spellthread", 3721, false}
    },
    [7] = {
        -- Boots
    ...
    [17] = {
        -- Ranged
        {"Ranged|r - Diamond-cut Refractor Scope", 3843, true},
        {"Ranged|r - Sun Scope", 3607, true},
        {"Ranged|r - Heartseeker Scope", 3608, true} -- {"Khorium Scope", 2723, false}
    }
}
local pVar = {}
...
function enchanterG_Enchanter(event, player)
    pVar[player:GetName()] = nil
    for event, v in ipairs(T["Menu"]) do
        player:GossipMenuAddItem(10, ""..TextColor.."Enchant|r " .. v[1] .. ".", 0, v[2])
    end
    player:GossipSendMenu(1, player, 700000)
end

function OnSelect(event, player, _, sender, intid, code)
    if (intid < 500) then
        local ID = intid
        local f
...
RegisterPlayerGossipEvent(700000, 2, OnSelect)